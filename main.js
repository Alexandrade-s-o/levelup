...
const OUTFIT_CATALOG = [
    { id: 'basic', name: 'Basico', price: 0 },
    { id: 'ninja', name: 'Ninja', price: 400 },
    { id: 'royal', name: 'Royal', price: 700 },
    { id: 'cyber', name: 'Cyber', price: 1000 }
];
...
class AndradeApp {
    ...
    loadLocalState() {
        ...
        this.state.avatar = { ...defaults.avatar, ...(localRaw.avatar || {}) };
        this.state.avatar.owned = Array.isArray(this.state.avatar.owned) ? this.state.avatar.owned : ['basic'];
        if (!this.state.avatar.owned.includes('basic')) this.state.avatar.owned.push('basic');
        if (!this.state.avatar.owned.includes(this.state.avatar.equipped)) this.state.avatar.equipped = 'basic';
        if (!Number.isFinite(this.state.coins)) this.state.coins = 0;
    ...
    renderAll() {
        ...
        this.$('dash-coins').innerText = s.coins.toLocaleString();
    ...
    awardXP(val) {
        ...
        if (newLvl > previousLevel) {
            const levelsWon = newLvl - previousLevel;
            const coinReward = levelsWon * 250;
            this.state.level = newLvl;
            this.state.coins += coinReward;
            this.notify(`🌟 NIVEL ${newLvl} · +${coinReward} monedas`);
        } else {
            this.state.level = newLvl;
        }
    ...
    getOutfitOverlaySvg(outfitId) {
        const overlays = {
            ninja: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 120"><path d="M20 55h80v18H20z" fill="#141414"/><circle cx="45" cy="64" r="5" fill="#00f2ff"/><circle cx="75" cy="64" r="5" fill="#00f2ff"/></svg>`,
            royal: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 120"><path d="M20 94h80v12H20z" fill="#4d3b1f"/><path d="M26 50l14 12 20-26 20 26 14-12v44H26z" fill="#f4d03f"/><circle cx="60" cy="60" r="7" fill="#00f2ff"/></svg>`,
            cyber: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 120"><path d="M16 62h88v16H16z" fill="#3d5afe" fill-opacity=".75"/><rect x="28" y="68" width="64" height="4" fill="#00f2ff"/><path d="M30 90h60v12H30z" fill="#111827"/></svg>`
        };
        return overlays[outfitId] || '';
    }
    ...
    renderAvatar() {
        const avatarBase = this.$('avatar-base');
        const avatarOutfit = this.$('avatar-outfit');
        if (!avatarBase || !avatarOutfit) return;

        const base = `https://api.dicebear.com/7.x/bottts-neutral/svg?seed=${this.uid}&backgroundColor=0f172a,1e293b`;
        avatarBase.src = base;

        const equipped = this.state.avatar.equipped || 'basic';
        const outfit = this.getOutfitById(equipped);
        this.$('avatar-outfit-name').innerText = outfit.name;

        const overlaySvg = this.getOutfitOverlaySvg(equipped);
        if (overlaySvg) {
            avatarOutfit.src = this.toDataSvg(overlaySvg);
            avatarOutfit.classList.remove('hidden');
        } else {
            avatarOutfit.removeAttribute('src');
            avatarOutfit.classList.add('hidden');
        }
    }

    renderAvatarShop() {
        const c = this.$('avatar-shop');
        if (!c) return;
        c.innerHTML = '';
        OUTFIT_CATALOG.forEach((item) => {
            const owned = this.state.avatar.owned.includes(item.id);
            const equipped = this.state.avatar.equipped === item.id;
            ...
            row.innerHTML = `
                <div>
                    ...
                    <div class="shop-item-price">${item.price === 0 ? 'Incluido' : `${item.price} monedas`}</div>
                </div>
                ${equipped
                    ? '<button class="btn-ghost shop-btn" disabled>Activo</button>'
                    : owned
                        ? `<button class="btn-primary shop-btn" onclick="app.equipOutfit('${item.id}')">Equipar</button>`
                        : `<button class="btn-ghost shop-btn" ${canBuy ? '' : 'disabled'} onclick="app.buyOutfit('${item.id}')">${canBuy ? 'Comprar' : 'Sin saldo'}</button>`
                }`;
            c.appendChild(row);
        });
    }
    ...
    async buyOutfit(id) {
        const outfit = this.getOutfitById(id);
        if (!outfit || this.state.avatar.owned.includes(id)) return;
        if (this.state.coins < outfit.price) {
            this.notify('No tienes monedas suficientes');
            return;
        }
        this.state.coins -= outfit.price;
        this.state.avatar.owned.push(id);
        this.state.avatar.equipped = id;
        this.notify(`🛍️ Compraste ${outfit.name}`);
        await this.save();
    }

    async equipOutfit(id) {
        if (!this.state.avatar.owned.includes(id)) return;
        this.state.avatar.equipped = id;
        this.notify(`Avatar equipado: ${this.getOutfitById(id).name}`);
        await this.save();
    }
